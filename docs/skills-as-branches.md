...
The upstream repo (`ridermw/nanoclaw`) maintains:
...
- Live in the marketplace repo (`ridermw/nanoclaw-skills`)
...
The marketplace repo uses the plugin structure:

```
ridermw/nanoclaw-skills/
  .claude-plugin/
...
1. Fork `ridermw/nanoclaw` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
...
Users who previously ran `git clone https://github.com/ridermw/nanoclaw.git` and have local customizations:

1. Fork `ridermw/nanoclaw` on GitHub
2. Reroute remotes:
   ```bash
...
3. From this point, `origin` = their fork, `upstream` = ridermw/nanoclaw
...
Run `/add-discord` (discovered via the marketplace plugin), or manually:
...
# Create PR from my-fix to ridermw/nanoclaw:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`ridermw/nanoclaw-skills`)
...
- `upstream` — `ridermw/nanoclaw` (core updates)
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/ridermw/nanoclaw.git`
- Check if `origin` points to the user's fork (not ridermw). If it points to ridermw, guide them through the fork migration.
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-nanoclaw, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `copilot plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`ridermw/nanoclaw-skills`) — single plugin bundling SKILL.md files for all feature skills
...
>
> **Discovering skills:** Skills are now available through the plugin marketplace. Run `/plugin` to browse and install available skills.
